...
async def executor(client: ChampuBot, message: Message):
...
@ChampuBot.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@ChampuBot.on_cb("forceclose")
...
async def shellrunner(client: ChampuBot, message: Message):
    if len(message.command) < 2:
