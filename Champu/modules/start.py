...
async def set_group_language(chat: Chat):
    ...
    async for message in Champu.get_chat_history(chat.id, limit=50):
...
async def welcomejej(client, message: Message):
    ...
    try:
        for member in message.new_chat_members:
            ...
            if member.id == Champu.id:
                try:
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"sᴇʟᴇᴄᴛ ʟᴀɴɢᴜᴀɢᴇ", callback_data="choose_lang")]])    
                    await message.reply_photo(photo=random.choice(IMG), caption=START.format(Champu.mention or "can't mention", users, chats), reply_markup=reply_markup)
                    chat = message.chat
                    ...
                    pass
                try:
                    invitelink = await Champu.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await Champu.download_media(
                ...
                count = await Champu.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await Champu.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(Champu.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await Champu.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@Champu.on_cmd("help")
async def help(client: Champu, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{Champu.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve 🥀 αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||**๏ мαdє ωιтн ❣️ ву [ᴠɪᴘ ʙᴏʏ](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@Champu.on_message(filters.command("stats"))
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await Champu.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await Champu.send_message(
